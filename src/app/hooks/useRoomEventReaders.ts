import { ReceiptType, Room, RoomEvent, RoomEventHandlerMap } from 'matrix-js-sdk';
...
const getEventReaders = (room: Room, evtId?: string) => {
    ...
    for (let i = liveEvents.length - 1; i >= 0; i -= 1) {
        userIds.splice(userIds.length, 0, ...room.getReceiptsForEvent(liveEvents[i]).filter(x => x.type !== ReceiptType.ReadPrivate).map(x => x.userId));
