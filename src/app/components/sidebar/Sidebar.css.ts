...
import { config, DefaultReset, Disabled, FocusOutline, toRem } from 'folds';
...
export const Sidebar = style([
	DefaultReset,
	{
		...
		borderRight: `${config.borderWidth.B300} solid var(--mui-palette-divider)`,
...
export const DropTarget = style({
	...
	selectors: {
		...
		'&[data-drop-child=true]': {
			outline: `${config.borderWidth.B700} solid `,
...
export const SidebarFolder = recipe({
	base: [
		ContainerColor({ variant: 'surface' }),
		{
			...
			display: 'flex',
			flexWrap: 'wrap',
			outline: `${config.borderWidth.B300} solid var(--mui-palette-divider)`,
			position: 'relative',
			selectors: {
				'button&': {
					cursor: 'pointer',
